...
/**
 * This object represents a SESSION parameter.
 * 
 * @author Pierre Vaidie
 */
class SessionParam implements ScalarValueInterface, ArrayValueInterface {
	...
	/**
	 * @Important $paramName
	 * @param string $paramName The SESSION parameter this object represents
	...
	public function val() {
		if (isset($_SESSION[$this->paramName])) {
			$value = $_SESSION[$this->paramName];
			if (get_magic_quotes_gpc()==1)
			{
				// we check first for arrays
				if (is_array($value)){
					if (!array_walk_recursive($value, "stripslashes")) {
						throw new \Exception('<b>Error!</b> An error occured while walking SESSION array "'.$this->paramName.'".');
					}
